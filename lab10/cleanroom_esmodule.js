...
// Components, render queue and timer
...
function useEffect(callback, deps) {
  ...
  // Store the effect and its dependencies
  if (componentEffects[index] === undefined) {
    componentEffects[index] = { cleanup: null, deps: null };
  }
...
function createSignal(initialValue) {
  ...
  // For internal use to track dependencies
...
function useSignal(initialValue) {
  ...
  return [state, setSignal];
}

// HTM binding to createElement
function html(...args) {
  return build.apply(createElement, args);
}
